...
#CueSelectorExpr: #NonEmptyString & =~"^[_#A-Za-z][_A-Za-z0-9]*(\\.[_#A-Za-z][_A-Za-z0-9]*)*$"
...
#VisibilityTier: "public" | "internal" | "restricted"
	...
	_operationRefProof: {
		for operationID, operation in operations {
			...
			for resourceID, _ in operation.creates {
				...
				"\(operationID)-creates-\(resourceID)-role": resources[resourceID] & {role: #GeneratedOutputResourceRole}
			}
...
#MakeNegativeFixtureProbeBinding: {
	...
	let builtFixture = (#MakeUncheckedNegativeFixture & {"in": in}).out
