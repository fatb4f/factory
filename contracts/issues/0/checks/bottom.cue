...
_negativeBottomChecks: {
	generatedArtifactsAuthorityAccepted?: (impl.#MakeBottomCheckProof & {
		in: {
			...
			input: {
				evidence: "generator projection authority is inadmissible"
				value: {
					sliceID:                        "factory.meta-generated-contract-compliance"
					issueNumber:                    0
					parentAuthority:                "contracts/meta"
					usesMetaConstructors:           true
					usesBottomCheckProof:           true
			...
			target: {
				...
				contract: {
					evidence: "issue slice rejects generator projection authority"
	...
	staleLocalCheckAccepted?: (impl.#MakeBottomCheckProof & {
		in: {
			...
			input: {
				evidence: "old issue-local check references are inadmissible"
				value: {staleIssueLocalChecks: true}
			}
			target: {
				name: "#IssueValidatorBoundary"
				contract: {
					evidence: "validator boundary rejects old issue-local checks"
	...
	externalLookupAccepted?: (impl.#MakeBottomCheckProof & {
		in: {
			...
			input: {
				evidence: "outside lookup authority is inadmissible"
				value: {externalLookupAuthority: true}
			}
			target: {
				name: "#IssueValidatorBoundary"
				contract: {
					evidence: "validator boundary rejects outside lookup authority"
					value:    #IssueValidatorBoundary
				}
			}
		}
