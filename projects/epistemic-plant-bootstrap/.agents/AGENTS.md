...
This directory is execution procedure only. Semantic authority remains in `contracts/workers/upstream-monitor/` and the selected `profiles_epistemic_plant_bootstrap/` package. The single generic worker procedure is `contracts/workers/upstream-monitor/AGENTS.md`.

When invoked:

1. Read `contracts/workers/upstream-monitor/contract.cue`, every current CUE file in `contracts/workers/upstream-monitor/profiles_epistemic_plant_bootstrap/`, and `contracts/workers/upstream-monitor/AGENTS.md` in the authority order declared by the generic worker procedure.
