...
The dispatcher is only a scheduler and ledger. Read `registry.cue`, run enabled tasks that are due according to `cadence.everyDays` and scheduler state, follow each task's declared `AGENTS.md`, then update that task's ledger after it returns its task-native outcome.
